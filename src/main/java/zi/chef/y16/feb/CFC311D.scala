...
/**
  ...
  */
object CFC311D extends App {
  val lines = scala.io.Source.fromInputStream(System.in).getLines()
  val n = lines.next().toInt

  val p = 95542721
  val tree = new LazySegmentTree[Long, Long, Int](
    lines.next().split(" ").map(_.toLong),
    x => x,
    (x, y) => (x + y) % p,
    (l, u) => {
      var out = l * l
  ...
  for (j <- 1 to q) {
    ...
    op match {
      case 2 => tree.lazyUpdate(l - 1, r, j)
      case 1 => println(tree.query(l - 1, r))
    }
  }
