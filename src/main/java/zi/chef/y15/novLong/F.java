...
//Task1 execution to check the understanding
public class F {

	public static void main(String[] args) throws Exception {
		System.setIn(new FileInputStream("/Users/balaudt/Temp/F.in"));
		...
		for (int i = 0; i < t; i++) {
			...
			Set<Set<Integer>> offers = new HashSet<>(m);
			for (int j = 0; j < m; j++) {
				inStr = reader.readLine().split(" ");
				Set<Integer> offer = new HashSet<>();
				for (int k = 1; k < inStr.length; k++) {
					offer.add(Integer.parseInt(inStr[k]) - 1);
				}
				offers.add(offer);
			}
			...
			while (permutation.hasNext()) {
				int curPrize = totalSum;
				int[] positions = permutation.next();
				int[] ain = new int[n];
				for (int j = 0; j < n; j++) {
					ain[j] = a[positions[j]];
				}
				...
				for (Set<Integer> offer : offers) {
					int minInd = offer.iterator().next();
					for (Integer item : offer) {
						if (ain[item] < ain[minInd])
							minInd = item;
					}
					if (!bought.get(minInd)) {
						curPrize -= ain[minInd];
						for (Integer item : offer) {
							bought.set(item);
						}
				...
				System.out.println(curPrize);

				if (curPrize < minPrize)
					minPrize = curPrize;
			}
