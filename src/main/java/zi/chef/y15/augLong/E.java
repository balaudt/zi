...
public class E {
	...
	public static void main(String[] args) throws Exception {
		...
		int r = random.nextInt(n - l);
		bitSet.set(l, l + r + 1);
		System.out.println(bitSet);
		int shifter = l + r - random.nextInt(r);
		bitSet.shiftRight(shifter);
		System.out.println(shifter + "\t" + bitSet);
	}
	...
	public static void execute(String[] args) throws Exception {
		...
		List<Num> nums = new ArrayList<>(n);
		for (int i = 0; i < n; i++) {
			int nextNo = Integer.parseInt(inStr[i]);
			Num newNum = new Num(nextNo, i);
			int ip = Collections.binarySearch(nums, newNum);
			if (ip > 0) {
				newNum = nums.get(ip);
				nums.get(ip).isMaxCountOdd ^= true;
			} else {
				ip = -ip - 1;
				nums.add(ip, newNum);
			}
			int size = nums.size(), newManCount = i, lastGreatInd = -1;
			for (int j = size - 1; j > ip && newManCount > 0; j--) {
				Num greaterNo = nums.get(j);
				if (greaterNo.ind < lastGreatInd)
					continue;
				int toAdd = greaterNo.ind - lastGreatInd - 1;
				if (toAdd > newManCount)
					toAdd = newManCount;
				newManCount -= toAdd;
				greaterNo.isMaxCountOdd ^= (toAdd % 2 != 0);
				lastGreatInd = greaterNo.ind;
			}
			if (newManCount > 0) {
				newNum.isMaxCountOdd ^= (newManCount % 2 != 0);
			}
		}
		...
		for (int i = 0; i < m; i++) {
			...
			int ip = Collections.binarySearch(nums, new Num(lk));
			boolean isDev = inStr[2].equals("D");
			if (inStr[0].equals("=")) {
				if (ip < 0) {
					printWinner(false, isDev);
				} else {
					if (ip == 0)
						printWinner(nums.get(0).isMaxCountOdd, isDev);
					else
						printWinner(nums.get(ip).isMaxCountOdd ^ nums.get(ip - 1).isMaxCountOdd, isDev);
				}
			} else if (inStr[0].equals("<")) {
				if (ip < 0)
					ip = -ip - 2;
				else
					ip--;
				if (ip < 0)
					...
					printWinner(nums.get(ip).isMaxCountOdd, isDev);
				...
				if (ip < 0)
					...
					left = nums.get(ip).isMaxCountOdd;
				printWinner(left ^ nums.get(size - 1).isMaxCountOdd, isDev);
			}
		}
		reader.close();
	}
...
class Num implements Comparable<Num> {
	...
	public String toString() {
		return "N[n=" + num + ",i=" + ind + ",f=" + (isMaxCountOdd ? 1 : 0) + "]";
	}
}
...
class BitSet implements Cloneable, java.io.Serializable {
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code BitSet.valueOf(longs).get(n) == ((longs[n/64] & (1L<
	 * <(n%64))) != 0)} <br>for all {@code n < 64 * longs.length}.
	 *
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code BitSet.valueOf(lb).get(n) == ((lb.get(lb.position()+n/64) & (1L<
	 * <(n%64))) != 0)} <br>for all {@code n < 64 * lb.remaining()}.
	 *
	 * <p>The long buffer is not modified by this method, and no reference to
	 * the buffer is retained by the bit set.
	 *
	 * @param lb
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code BitSet.valueOf(bytes).get(n) == ((bytes[n/8] & (1<
	 * <(n%8))) != 0)} <br>for all {@code n <  8 * bytes.length}.
	 *
	 * <p>This method is equivalent to
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code BitSet.valueOf(bb).get(n) == ((bb.get(bb.position()+n/8) & (1<
	 * <(n%8))) != 0)} <br>for all {@code n < 8 * bb.remaining()}.
	 *
	 * <p>The byte buffer is not modified by this method, and no reference to
	 * the buffer is retained by the bit set.
	 *
	 * @param bb
	 ...
	 * {@code s.get(n) == ((bytes[n/8] & (1<<(n%8))) != 0)} <br>for all
	 * {@code n < 8 * bytes.length}.
	 *
	 ...
	 * {@code s.get(n) == ((longs[n/64] & (1L<<(n%64))) != 0)} <br>for all
	 * {@code n < 64 * longs.length}.
	 *
	...
	public void shiftRight(int startIndex) {
		int wordIndex = wordIndex(startIndex);
		startIndex &= 0x3F;
		long lastBit = words[wordIndex] & 1;
		long value = words[wordIndex] & (WORD_MASK >>> startIndex);
		words[wordIndex] &= (WORD_MASK >>> startIndex) ^ WORD_MASK;
		value = value >>> 1;
		words[wordIndex] |= value;
		for (int i = wordIndex + 1; i < wordsInUse; i++) {
			if (words[i] == 0 && lastBit == 0)
				break;
			boolean wasLastSet = (lastBit == 1);
			lastBit = words[i] & 1;
			words[i] >>>= 1;
			if (wasLastSet) {
				words[i] |= 1 << 63;
