...
public class ReorderedPower {
	...
	public boolean reorderedPowerOf2(int N) {
		...
		Permutation permutation = new Permutation(digits.length, digits.length);
		while (permutation.hasNext) {
			int[] nextPerm = permutation.next();
			if (digits[nextPerm[0]] == '0') continue;
			char[] permutedDigits = new char[digits.length];
			for (int i = 0; i < digits.length; i++) {
				permutedDigits[i] = digits[nextPerm[i]];
			}
			int permutedNum = Integer.parseInt(new String(permutedDigits));
			if ((permutedNum & permutedNum - 1) == 0) return true;
		}
		return false;
	}

	public static void main(String[] args) {
		ReorderedPower reorderedPower = new ReorderedPower();
		System.out.println(reorderedPower.reorderedPowerOf2(1));
		System.out.println(reorderedPower.reorderedPowerOf2(10));
		System.out.println(reorderedPower.reorderedPowerOf2(16));
		System.out.println(reorderedPower.reorderedPowerOf2(24));
		System.out.println(reorderedPower.reorderedPowerOf2(46));
	}
}
